...
class LLMBrainConfig:
    # backend: "llama_cpp" (default, loads GGUF directly) or "ollama"
    # (HTTP to localhost:11434 — recommended on Jetson since Ollama is
    # already there and manages the blob store).
    backend: str = "llama_cpp"
    model_path: str = ""                        # llama_cpp: absolute GGUF path
    model_name: str = "llama-3.1-8b-q4"          # ollama: tag like "llama3.1:8b"
    enabled: bool = False
...
class LLMBrain:
    ...
    def _ensure_model(self) -> bool:
        ...
        with self._lock:
            ...
            # ----- Ollama backend: HTTP client, no GGUF path needed -----
            if self.cfg.backend == "ollama":
                try:
                    ...
                    if not client.ping():
                        ...
                        _log.warning("llm_brain_ollama_unreachable base_url=%s",
                                      client.cfg.base_url)
                        return False
                    self._model = client
                    _log.info("llm_brain_loaded backend=ollama tag=%s",
                               self.cfg.model_name)
                    return True
                except Exception as e:
                    self._load_err = f"ollama_init_failed:{type(e).__name__}:{e}"
                    _log.warning("llm_brain_ollama_init_failed err=%s", e)
                    return False

            # ----- llama_cpp backend: load GGUF from disk -----
            ...
            try:
                ...
                _log.info("llm_brain_loaded backend=llama_cpp model=%s",
                           self.cfg.model_name)
                return True
            except Exception as e:
    ...
    def _infer(self, prompt: str) -> str:
        """Raw one-shot completion. Dispatches to whichever backend the
        model was loaded with."""
        assert self._model is not None

        # Ollama backend — OllamaClient in self._model
        if self.cfg.backend == "ollama":
        ...
        # llama_cpp backend — Llama instance in self._model
...
def build_llm_brain_from_settings(settings) -> Optional[LLMBrain]:
    ...
    # Model name — for ollama, this is the tag (e.g. "llama3.1:8b").
