...
def test_brain_uses_ollama_backend_when_configured(monkeypatch):
    """With backend=ollama, the brain should construct an OllamaClient
    (not load a GGUF) and dispatch _infer through client.generate."""
    from src.intelligence import llm_brain as mod
    from src.intelligence.llm_brain import LLMBrain, LLMBrainConfig

    # Force the factory to return a stub OllamaClient that always
    # responds. No real HTTP.
    class _StubClient:
        class cfg:
            base_url = "http://127.0.0.1:11434"
        def ping(self): return True
        def generate(self, **kw):
            return ('{"action":"adjust","confidence_multiplier":0.9,'
                    '"reason":"ollama-driven"}')

    monkeypatch.setattr(
        "src.intelligence.ollama_client.build_ollama_client",
        lambda: _StubClient(),
    )
    brain = LLMBrain(LLMBrainConfig(
        enabled=True, backend="ollama",
        model_name="llama3.1:8b",
        rate_limit_sec_per_symbol=0.0,   # disable for test
    ))
    result = brain.review(_candidate(), _ctx())
